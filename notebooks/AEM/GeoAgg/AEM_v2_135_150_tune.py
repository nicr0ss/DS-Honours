...
# Load data
grid = pd.read_parquet('../deeper/PercentCoarse_135_150.parquet')
print(len(grid))

grid = grid.copy()
grid["Coarse_mid"] = grid.PercentCoarse_135_150
grid = grid.drop(columns=["PercentCoarse_135_150"])
...
def objective(trial, n_split=5):
    ...
    with open(save_dest + "results_v2.txt", "a") as f:
    ...
    def train_and_evaluate_fold(trn_idx, val_idx):
        ...
        """
        with open(save_dest + "nodes_v2.txt", "a") as f:
        ...
        with open(save_dest + "r2_v2.txt", "a") as f:
